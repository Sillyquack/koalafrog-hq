-- Harden the Foot Care handoff so the database, rather than the browser,
-- attests registry membership and benchmark provenance. Every candidate group
-- is validated before the first write, and persisted provenance is derived
-- from this immutable registry snapshot.
...
begin
  ...
  if jsonb_typeof(concept.analysis->'footCare') is distinct from 'object' then
    raise exception 'FOOT_CARE_HANDOFF_CONCEPT_ANALYSIS_INVALID';
  end if;

  concept_registry_version := nullif(trim(concept.analysis->'footCare'->>'registryVersion'), '');
  project_kind := nullif(trim(concept.analysis->'footCare'->>'projectKind'), '');
