...
begin
  ...
  result_state:=case when exists(select 1 from public.purchase_order_receipt_discrepancies where receipt_id=r.id and resolution_status in('open','under_review')) then 'discrepancy_review'
    ...
    when exists(select 1 from public.purchase_order_receipt_lines where receipt_id=r.id and (identity_status='mismatch' or condition_status in('damaged','critical_concern','held') or not coalesce((documentation_checks->>'documentationComplete')::boolean,false))) then 'discrepancy_review'
...
begin
  ...
  if exists(select 1 from public.purchase_order_receipt_lines where receipt_id=r.id and (identity_status='mismatch' or condition_status='critical_concern')) then raise exception 'CRITICAL_RECEIVING_CHECK_BLOCKS_QUARANTINE'; end if;
