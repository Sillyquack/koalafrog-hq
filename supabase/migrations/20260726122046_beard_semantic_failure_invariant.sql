-- Reconcile the durable provider budget with the bounded v23 runtime and make
-- semantic failure metadata complete. Historical rows are not rewritten.

alter table public.intelligence_analyses
  drop constraint intelligence_analyses_provider_timeout_budget_check,
  add constraint intelligence_analyses_provider_timeout_budget_check check (
    provider_timeout_budget_ms is null or
    provider_timeout_budget_ms between 60000 and 300000
  ),
  add column failure_recommendation_index integer,
  add constraint intelligence_analyses_failure_recommendation_index_check check (
    failure_recommendation_index is null or failure_recommendation_index >= 0
  );
...
begin
  ...
  if candidate_provider <> 'openai'
    ...
    or candidate_prompt_version <> 'beard-photo-analysis-v5' then
    return false;
  end if;
...
begin
  ...
  select jsonb_build_object(
    ...
    'errorCode',case when a.error_code in (
      ...
      'SEMANTIC_VALIDATION_FAILED','SEMANTIC_VALIDATOR_INTERNAL_ERROR',
      'UNKNOWN_VALIDATION_FAILURE','INVALID_IMAGE',
      'STORAGE_AUTHORIZATION_FAILED','PROVIDER_FAILURE',
    ...
    'recommendationIndex',a.failure_recommendation_index,
