...
The earlier compatibility importer stored one JSON object per domain record in `workspace_records`. The relational destination uses 50 explicit domain tables and five relational child/join tables. Composite workspace-scoped foreign keys preserve the graph, while snapshot columns preserve historical meaning. Local v9 is the development default; an explicitly reconciled and activated Supabase workspace can be selected as authoritative at startup.

Phase 8B.2 does not change domain entities. Its action executor treats each domain transition as an intended next workspace state and publishes it only after the selected repository confirms persistence. Append-only and snapshot semantics therefore remain domain rules independent of the Local or relational adapter.

The five normalized child groups retain their domain shapes after relational hydration. Template questions are ordered mutable-template children; submitted response answers and Compliance composition lines are immutable history captured with their parents; Regulatory Review sources and PIF document references are mutable parent-scoped join sets. Legitimate mutable-link replacement deletes only the selected parent's old child rows before inserting the intended set, so refresh does not reconstruct stale links or orphans.

`ComplianceDocument` describes evidence even when no file is attached. Zero or more `document_objects` retain its binary history. A new attachment becomes `Current` and supersedes the prior version; removal changes Current metadata to `Removed` and deletes its private object while retaining audit metadata. Each object carries workspace/owner, Dossier and Document IDs, bucket/path, original name, MIME type, bytes, uploader, timestamps, version, lifecycle state, and optional checksum. Binary content is never part of the domain aggregate or JSON backup.
