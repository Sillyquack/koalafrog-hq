...
The earlier compatibility importer stored one JSON object per domain record in `workspace_records`. Phase 8B.1 supersedes that destination with 50 explicit domain tables and five relational child/join tables. Composite workspace-scoped foreign keys preserve the graph, while snapshot columns preserve historical meaning. The application still deliberately defaults to local v9 until the separate Phase 8B.3 authoritative startup cutover.
