...
| TD-013 | Hosted grants, RLS, private Storage metadata, advisors, and two-owner isolation were verified; managed Auth restore and application acceptance remain incomplete | Partial parity may be mistaken for complete hosted readiness | Default-execute defect fixed locally and on the isolated target; rehearsal remains BLOCKED | Hosted rehearsal PASS with managed Auth restoration and application smoke |
...
Closeout status: TD-001 through TD-010 remain open and non-blocking for merge review. TD-007, TD-009, and TD-010 continue to block deployment. The hosted rehearsal resolved the internal definer-helper default-execute defect but did not satisfy managed Auth restoration. No closed item was deleted.
