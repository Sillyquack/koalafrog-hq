...
Phase 8B.2 routes the 59 provider commands through a persistence-confirmed action executor and a session-selected repository. The Local adapter remains the delivered default and is the only code that writes the v9 aggregate. The Supabase adapter provides relational hydration, ordinary entity mutations, and stale-write detection, but refuses audit-critical multi-table operations until their dedicated transactional RPCs are implemented.
