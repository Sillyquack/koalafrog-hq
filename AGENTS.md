...
- Store compliance document metadata relationally and binaries only in the private `compliance-documents` bucket. Never store Base64 blobs or public URLs. Keep superseded/removed version metadata for audit history. CPSR conclusions and CPNP confirmation require real external evidence metadata.
...
- Phase 8 relational tables are the Supabase source of truth after an explicitly reconciled workspace is activated. Local v9 remains the rollback source; do not add domain writes to `workspace_records`.
- Persistent UI commands must use the workspace action executor and the single startup-selected repository. Never reintroduce provider save effects, runtime UI switching, silent fallback, or dual-write Local/Supabase behaviour. `VITE_WORKSPACE_REPOSITORY=supabase` is an explicit controlled cutover; Local remains the development default.
