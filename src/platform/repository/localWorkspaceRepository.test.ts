...
describe('LocalWorkspaceRepository', () => {
  ...
  it('round-trips Natural Deodorant concepts through existing v9 local storage',()=>{const values=new Map<string,string>(),process=[{order:1,title:'Review',instruction:'Review evidence.',critical:false,completionCriteria:'Reviewed.'}];vi.stubGlobal('window',{localStorage:{getItem:(key:string)=>values.get(key)??null,setItem:(key:string,value:string)=>values.set(key,value)}});const repository=new LocalFormulaRepository(),state=structuredClone(formulaSeed);state.productStudioConcepts.push({id:'natural-deodorant-local',name:'Local fixture',productType:'natural_deodorant',intentMode:'design',desiredProperties:['Jar'],selectedIngredients:[],scentDirections:['Fragrance-free'],candidateSubstitutes:{},notes:'Context only',analysis:{packagingIntent:'Jar',manufacturingProcess:process},createdAt:'2026-07-20',updatedAt:'2026-07-20'});repository.save(state);expect(repository.load().productStudioConcepts[0]).toMatchObject({productType:'natural_deodorant',analysis:{packagingIntent:'Jar',manufacturingProcess:process}})})
})
