...
run("Packaging Run Planning & Control against local Supabase", () => {
  ...
  const fixture = async (label: string, retained = 100) => {
    ...
    for (const [index, component] of ["bottle","closure"].entries()) {
      ...
      expect((await admin.from("packaging_inventory_movements").insert({ ...owned, id: `${component}-receipt-${label}`, packaging_inventory_lot_id: lotId, type: "Receipt", quantity: 30, unit: "pcs", reason: "Test receipt", notes: "", occurred_at: now, created_at: now })).error).toBeNull();
    }
