...
import { activities, batches, testingActivities } from '../../data/mockData'
...
export function DashboardPage() {
  const { products, formulaVersions, ingredients, inventoryLots, inventoryMovements, supplierProducts } = useFormulaData()
  const active = products.filter((p) => p.status === 'Active')
  const lowStock = ingredients.filter((i) => ['Low','Out of Stock'].includes(stockState(i, inventoryLots, inventoryMovements)))
  const versionLabel = (id?: string) => formulaVersions.find((version) => version.id === id)?.version ?? '—'
  return <>
    ...
    <div className="dashboard-grid">
      <section className="panel span-8"><SectionHeader title="Active development" detail={`${active.length} products currently moving`} action={<button className="text-button">All products <ArrowRight size={15} /></button>} /><div className="development-list">{active.map((product) => <article key={product.id}><div className="dev-title"><span className="category-dot" /><div><strong>{product.name}</strong><small>{product.category}</small></div><StatusPill tone={product.developmentStage === 'Testing' ? 'blue' : product.developmentStage === 'Formulation' ? 'amber' : 'green'}>{product.developmentStage}</StatusPill></div><div className="dev-detail"><span>{versionLabel(product.currentDevelopmentFormulaVersionId)}</span><span>{product.scentProfile}</span><span>Updated {product.updatedAt.slice(5)}</span></div></article>)}</div></section>
      <section className="panel span-4 attention-panel"><SectionHeader title="Material attention" detail="Lot-derived stock signals" /><strong className="attention-number">{lowStock.length}</strong><span>materials at or below reorder</span><div>{lowStock.slice(0,4).map((item) => <p key={item.id}><span>{item.commonName}<small>{supplierProducts.find((p) => p.ingredientId === item.id && p.isPreferred)?.supplierName ?? 'No preferred source'}</small></span><b>{ingredientBalance(item, inventoryLots, inventoryMovements)} {item.defaultUnit}</b></p>)}</div><button className="text-button">Review ingredients <ArrowRight size={15} /></button></section>
