...
describe('Procurement follow-up research UI',()=>{
 ...
 it('shows the previous-job gaps, editable instructions, country and review lineage responsively',()=>{
  ...
  expect(panel).toContain('Tax / duty estimate:')
  expect(panel).toContain('candidate.unresolved_fields.map(researchFieldLabel)')
  expect(panel).toContain('resolved.map(researchFieldLabel)')
