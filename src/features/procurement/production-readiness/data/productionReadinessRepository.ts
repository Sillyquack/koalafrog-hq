...
export async function loadProductionRound(workspaceId:string,roundId:string):Promise<DurableRoundAggregate>{
  ...
  const [draftLinesResult,draftAuditResult,confirmationsResult,confirmationLinesResult,shipmentsResult,shipmentLinesResult,shipmentEventsResult,receiptsResult,receiptShipmentsResult,receiptLinesResult,receiptDiscrepanciesResult,receiptInspectionsResult,quarantineIntakesResult,qualityReleaseReviewsResult]=orderIds.length?await Promise.all([
    ...
    database.from('inventory_quality_release_reviews').select('*').eq('workspace_id',workspaceId).order('reviewed_at',{ascending:false}),
  ]):Array.from({length:14},()=>({data:[],error:null}))
  ...
  return {
    ...
    quarantineIntakes:(quarantineIntakesResult.data??[]).map(row=>numeric(row,['quarantine_quantity','released_quantity','rejected_quantity','revision'])) as unknown as DurableQuarantineIntake[],
    qualityReleaseReviews:(qualityReleaseReviewsResult.data??[]).map(row=>numeric(row,['review_version','disposition_quantity','total_acquisition_cost'])) as unknown as DurableQualityReleaseReview[],
  }
}
...
export async function reviewQuarantinedInventory(intake:DurableQuarantineIntake,payload:Record<string,unknown>,idempotencyKey=crypto.randomUUID()){
  const result=await client().rpc('review_quarantined_inventory',{target_quarantine_intake_id:intake.id,expected_intake_revision:intake.revision,candidate_idempotency_key:idempotencyKey,review_payload:payload as Json})
  if(result.error)throw new Error(result.error.message)
  return result.data as {reviewId:string;inventoryKind:string|null;inventoryLotId:string|null;openingMovementId:string|null;intakeRevision:number}
}
