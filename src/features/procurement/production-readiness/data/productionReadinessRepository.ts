...
export async function loadProductionRound(workspaceId:string,roundId:string):Promise<DurableRoundAggregate>{
  ...
  return {
    ...
    draftOrders:(draftOrdersResult.data??[]).map(row=>numeric(row,['source_purchase_plan_version','merchandise_subtotal','discount','shipping','tax','total','actual_merchandise_subtotal','actual_discount','actual_shipping','actual_vat','actual_import_vat','actual_duty','actual_customs','actual_handling','actual_grand_total','revision'])) as unknown as DurableDraftPurchaseOrder[],
    draftOrderLines:(draftLinesResult.data??[]).map(row=>numeric(row,['ordered_package_count','ordered_quantity','expected_unit_price','verified_unit_price','effective_unit_price','line_subtotal','actual_package_count','actual_unit_price','actual_line_subtotal'])) as unknown as DurableDraftPurchaseOrderLine[],
