...
describe('follow-up research objective',()=>{
 ...
 it('reports unresolved gaps, practical-candidate misses and newly resolved fields without overwriting the prior candidate',()=>{
  expect(unresolvedFieldsForCandidates([prior])).toEqual(['delivery_estimate_days','shipping_cost'])
  expect(requestedItemsWithoutPracticalCandidate(items,[prior]).map(item=>item.id)).toEqual(['item-1','item-2'])
  const followUp={...prior,id:'candidate-follow-up',research_job_id:'job-follow-up',follow_up_to_candidate_id:prior.id,shipping_cost:79,unresolved_fields:['delivery_estimate_days']}
  expect(fieldsResolvedByFollowUp(followUp,prior)).toEqual(['shipping_cost'])
  expect(prior.shipping_cost).toBeNull()
  expect(prior.unresolved_fields).toEqual(['shipping_cost','delivery_estimate_days'])
 })
})
