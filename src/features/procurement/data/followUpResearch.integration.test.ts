...
run('targeted Procurement follow-up research against local Supabase',()=>{
 ...
 it('creates an owner-scoped linked job, publishes pending lineage, and has zero downstream side effects',async()=>{
  ...
  expect(followUpJob.data.follow_up_context).toMatchObject({
   schemaVersion:1,unresolvedFields:['delivery_estimate_days','shipping_cost'],
   priorCandidates:[expect.objectContaining({id:priorCandidate.data.id,requestedItemId:surfactantItem.id,unresolvedFields:['shipping_cost','delivery_estimate_days']})],
  ...
  expect(linked.data).toMatchObject({review_status:'pending',follow_up_to_candidate_id:priorCandidate.data.id,shipping_cost:79,unresolved_fields:['delivery_estimate_days']})
  expect((await (supabase as any).from('procurement_offer_candidates').select('*').eq('id',priorCandidate.data.id).single()).data).toEqual(originalCandidate)

  const retry=await (supabase as any).from('procurement_research_jobs').insert({...owned,procurement_request_id:request.data.id,provider:'openai-web-search-v1',status:'queued',retry_of_job_id:priorJob.data.id}).select('retry_of_job_id,follow_up_of_job_id,follow_up_context,follow_up_instructions,live_research_consent_at').single()
  if(retry.error)throw retry.error
  expect(retry.data).toEqual({retry_of_job_id:priorJob.data.id,follow_up_of_job_id:null,follow_up_context:null,follow_up_instructions:null,live_research_consent_at:null})
