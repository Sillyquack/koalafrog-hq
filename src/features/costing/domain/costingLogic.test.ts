...
describe('milligram costing',()=>{it('prices 250 mg from a 100 g package costing 20',()=>{const sample={...supplier,packageQuantity:100,packageUnit:'g' as const,price:20,currency:'NOK',isPreferred:true};expect(estimateFormulaCost([line],250,'mg',[],[],[sample],'NOK',asOf).total).toBe(.05)});it('keeps production snapshot arithmetic stable in mg',()=>{const lines=[{id:'mg-line',ingredientNameSnapshot:'Powder'} as ProductionRunLine],allocations=[{id:'mg-a',productionRunLineId:'mg-line',quantity:250,unit:'mg',inventoryMovementId:'m',unitCostSnapshot:.0002} as ProductionRunAllocation];expect(actualMaterialCost(lines,allocations).total).toBe(.05)})})
