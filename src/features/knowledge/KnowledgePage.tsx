...
function ScentMemoryLibrary({sessions,refresh}:{sessions:ScentMemorySession[];refresh:()=>Promise<void>}){
  ...
  const preset=params.get("labBatchId")?`batch:${params.get("labBatchId")}`:params.get("formulaVersionId")?`formula:${params.get("formulaVersionId")}`:params.get("ingredientId")?`ingredient:${params.get("ingredientId")}`:"";
  ...
  const options=[...data.products.map(x=>({value:`product:${x.id}`,label:`Product · ${x.name}`})),...data.formulaVersions.map(x=>({value:`formula:${x.id}`,label:`Formula Version · ${x.version}`})),...data.labBatches.map(x=>({value:`batch:${x.id}`,label:`Lab Batch · ${x.batchNumber}`})),...data.ingredients.map(x=>({value:`ingredient:${x.id}`,label:`Ingredient · ${x.commonName}`})),...data.testSessions.map(x=>({value:`test:${x.id}`,label:`Test Session · ${x.name}`}))];
  const create=async()=>{if(!workspace||!title.trim()||!context)return;const [kind,id]=context.split(":");try{await createScentMemorySession({workspaceId:workspace.workspaceId,title,...(kind==="product"?{productId:id}:kind==="formula"?{formulaVersionId:id}:kind==="batch"?{labBatchId:id}:kind==="ingredient"?{ingredientId:id}:{testSessionId:id})});setTitle("");setContext("");setCreating(false);await refresh()}catch(cause){setMessage(cause instanceof Error?cause.message:"Scent Memory could not be created.")}};
