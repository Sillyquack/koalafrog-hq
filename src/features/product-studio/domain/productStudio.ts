...
export function beardOilHandoffIssues(input:{saved:boolean;selections:ProductStudioSelection[];lines?:Array<{percentage:number}>;findings:RuleFinding[]}){
 const issues:string[]=[]
 if(!input.saved)issues.push('Save the concept before creating a Formula.')
 if(!input.selections.length)issues.push('Add at least one valid workspace Ingredient.')
 if(input.selections.length&&!input.lines?.length)issues.push('The concept has no structured composition. Notes are not Formula ingredients.')
 if(input.lines?.some(line=>!Number.isFinite(line.percentage)||line.percentage<=0))issues.push('Assign a percentage to every selected Ingredient.')
 if(input.lines?.length&&Math.abs(input.lines.reduce((sum,line)=>sum+line.percentage,0)-100)>0.0001)issues.push('Formula percentages must total 100%.')
 if(input.findings.some(item=>item.severity==='blocking'))issues.push('Resolve the highlighted Ingredient before continuing.')
...
export function profileIngredient(ingredient:Ingredient):IngredientFit{
 const name=normalized(ingredient),category=ingredient.category.toLowerCase(),functions=ingredient.functions.map(item=>item.toLowerCase()),scent=/essential oil|scent material|scent blend|parfum/.test(name)||functions.some(item=>item==='fragrance material'||item==='perfuming'),solid=/\b(butter|starch|powder)\b/.test(`${ingredient.commonName} ${category}`.toLowerCase())||/^(wax|solid|powder)$/.test(category)
 ...
 return{ingredient,role,range:scent?[0,2]:antioxidant?[0.1,1]:castor?[2,20]:squalane?[10,60]:solid?[0,0]:[20,100],physicalState:powder?'powder':solid?'solid':'liquid',oilSoluble:!water&&!powder,richness:castor||butter||wax?5:squalane?1:jojoba?3:2,absorption:squalane?5:castor?1:jojoba?3:3,slip:squalane?5:castor?4:3,gloss:castor?5:squalane?2:3,oxidativeStability:jojoba||squalane?5:antioxidant?4:/hemp|flax|rosehip/.test(name)?1:3,nativeOdour:scent?5:/unrefined|neem/.test(name)?4:1,scentStrength:scent?5:0,scentFamily:/cedar|wood/.test(name)?'Woody':/bergamot|citrus/.test(name)?'Citrus':/cardamom|spice/.test(name)?'Spicy':undefined,note:/bergamot|citrus/.test(name)?'top':/cardamom/.test(name)?'heart':scent?'base':undefined,tags:[squalane?'lightweight':'',jojoba?'balanced':'',castor?'control':'',antioxidant?'oxidative-support':''].filter(Boolean),cautions:[solid?'Solid material: use a workflow with heat and phase controls rather than treating it as a simple liquid.':'',water?'Not compatible with a simple anhydrous oil system.':'',scent?'Strong aromatic material; product-specific review is needed.':'',/hemp|flax|rosehip/.test(name)?'Oxidation-sensitive material.':''].filter(Boolean),provenance:name.includes('jojoba')||squalane||castor||antioxidant||scent?'curated':'partially_profiled'}
}
...
export function analyzeCombination(selections:ProductStudioSelection[],ingredients:Ingredient[]):RuleFinding[]{
 ...
 const solids=fits.filter(fit=>fit.physicalState!=='liquid'||!fit.oilSoluble);if(solids.length)findings.push({id:'bo.solid-or-incompatible',severity:'blocking',title:'Selected material does not fit a simple liquid',explanation:`${solids.map(f=>f.ingredient.commonName).join(' and ')} ${solids.length===1?'requires':'require'} a different physical system. Solid materials can prevent a uniform room-temperature liquid and require controlled heating and cooling.`,evidence:solids.map(f=>`${f.ingredient.commonName}: ${f.physicalState}`),actions:['Remove the highlighted Ingredient','Replace it with a liquid oil-compatible Ingredient','Use the Beard Butter workflow for a heat-processed concept']})
