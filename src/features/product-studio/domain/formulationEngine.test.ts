...
describe('Core Formulation Engine registry',()=>{
  ...
  it('registers future archetypes as planned capabilities without operational templates',()=>{
    ...
    expect(Object.values(formulationArchetypes).filter(item=>item.maturity==='operational').map(item=>item.id)).toEqual(['simple_liquid','anhydrous_multiphase'])
    expect(Object.values(productTemplates).map(item=>item.archetypeId)).toEqual(['simple_liquid','anhydrous_multiphase'])
  })
