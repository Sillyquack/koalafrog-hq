...
export function ProductDetailPage() {
  ...
  const { products, formulas, formulaVersions, labBatches, productionRuns, testSessions, createFormula } = useFormulaData()
  ...
  const recentRuns=productionRuns.filter(run=>run.productId===product.id).slice(0,3)
  ...
  return <>
    ...
    <section className="panel linked-testing"><SectionHeader title="Production & cost context" detail="Controlled runs remain separate from experimental Lab Batches" action={<Link className="text-button" to="/costing">Open Costing</Link>}/>{recentRuns.map(run=><Link to={`/production/${run.id}`} key={run.id}><strong>{run.productionRunNumber} · {run.purpose}</strong><span>{run.status} · {run.actualUnitsProduced??'—'} units produced</span></Link>)}{!recentRuns.length&&<p className="empty-copy">No Production Runs reference this product.</p>}</section>
  </>
}
