...
export function StartBatchForm({ onClose }: { onClose: () => void }) {
  ...
  return <div className="modal-backdrop"><form className="workspace-modal" onSubmit={submit}><span className="eyebrow">Formula to execution snapshot</span><h2>Start Lab Batch</h2><div className="form-grid">
    ...
    <label>Planned size<input name="size" type="number" min="0.1" step="0.1" required /></label><label>Unit<select name="unit" defaultValue="g"><option>g</option><option>kg</option></select></label><label className="span-2">Purpose<input name="purpose" required /></label><label className="span-2">Notes<textarea name="notes" /></label>
