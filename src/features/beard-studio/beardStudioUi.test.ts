...
describe('Beard Studio integration', () => {
  ...
  it('keeps photo analysis explicit, reviewable, and separate from authoritative Beard data', () => {
    ...
    const durableLookup = readFileSync('src/features/beard-studio/components/BeardAnalysisHistory.tsx', 'utf8')
    ...
    expect(intelligence).toContain('BeardAnalysisHistory')
    expect(durableLookup).toContain('listBeardAnalysisHistory')
    expect(durableLookup).toContain('Open analysis')
    expect(durableLookup).toContain('navigator.clipboard')
    expect(flow).not.toMatch(/saveBeardStudio|localStorage|workspace_records/)
  })
  it('shows atomic review failures and scrolls each reopened analysis after render', () => {
    const flow = readFileSync('src/features/beard-studio/components/BeardPhotoAnalysisFlow.tsx', 'utf8')
    const history = readFileSync('src/features/beard-studio/components/BeardAnalysisHistory.tsx', 'utf8')
    expect(flow).toContain("setSaved('');setFinishError('')")
    expect(flow).toContain('Nothing was changed. Please try again.')
    expect(flow).toContain('finishError&&<p className="form-error" role="alert"')
    expect(history).toContain("scrollIntoView({block:'start'})")
    expect(history).toContain('[opened?.analysisId]')
    expect(history).toContain('setOpened(undefined);setLoading(true)')
  })
})
