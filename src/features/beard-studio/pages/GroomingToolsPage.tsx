...
export function GroomingToolsPage() {
  ...
  const save=async(event:FormEvent)=>{
    ...
    try{await update(current=>({...current,tools:[...current.tools.filter(item=>item.id!==tool.id).map(item=>tool.primary&&item.type===tool.type?{...item,primary:false}:item),tool]}));setEditing(null)}catch{/* shared action error remains visible; keep the draft open */}
  }
