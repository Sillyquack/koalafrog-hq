...
export function BeardProfilePage() {
  ...
  const save = (event: FormEvent<HTMLFormElement>) => {
    ...
    void update(current => ({ ...current, profiles: current.profiles.some(item => item.id === editing.id) ? current.profiles.map(item => item.id === editing.id ? { ...editing, updatedAt: beardStudioNow() } : item) : [...current.profiles, editing] }))
