...
export function TrimRecipesPage() {
  ...
  const save=async()=>{
    ...
    try{await update(current=>({...current,recipes:[...current.recipes.filter(item=>item.id!==editing.id),{...editing,updatedAt:beardStudioNow()}]}));setEditing(null)}catch{/* shared action error remains visible; keep the draft open */}
  }
