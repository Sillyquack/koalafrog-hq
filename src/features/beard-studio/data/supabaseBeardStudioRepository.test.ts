...
describe('hosted Beard Studio persistence', () => {
  ...
  it('builds one atomic payload for profiles, tools, maps, recipes, sessions, logs and Product snapshots', () => {
    ...
    expect(payload).toMatchObject({workspace_id:'workspace',profiles:[{status:'Active'}],tools:[{attachments:expect.any(Array)}],length_maps:[{zones:expect.any(Array)}],recipes:[{preferred_products:[{product_id:'product-oil',role:'beard oil'}]}],sessions:[],logs:[]})
  })
