...
import { beardStudioPayload, mapBeardStudioRows, type StudioRows } from './beardStudioRepository'
...
describe('hosted Beard Studio persistence', () => {
  ...
  it('keeps persistence mapping free of an independent runtime selector',async()=>{const source=await import('./beardStudioRepository');expect(source).not.toHaveProperty('selectBeardStudioRepository');expect(source).not.toHaveProperty('LocalBeardStudioRepository')})
})
