...
import { groomingProductRoles, type BeardLengthMap, type BeardLogSnapshot, type BeardProfile, type BeardStudioState, type BeardTrimSession, type GroomingProductReference, type GroomingTool, type TrimRecipe } from '../domain/beardStudio'
...
export function beardStudioPayload(workspaceId: string, state: BeardStudioState,expectedRevision=state.revision): Json {
...
export class SupabaseBeardStudioGateway {
  ...
  async load() {
    const responses = await Promise.all([
      ...
      this.client.from('beard_studio_revisions').select('revision').eq('workspace_id',this.workspaceId).maybeSingle(),
    ])
    const failed=responses.find(response=>response.error)
    if(failed?.error)throw new Error(`Beard Studio could not be loaded: ${failed.error.message}`)
    const [profileResponse,toolResponse,attachmentResponse,mapResponse,zoneResponse,recipeResponse,stepResponse,sessionResponse,logResponse,recipeProductResponse,logProductResponse,revisionResponse]=responses
    return mapBeardStudioRows({
      ...
      revision:revisionResponse.data?.revision??0,logs:logResponse.data??[],recipeProducts:recipeProductResponse.data??[],logProducts:logProductResponse.data??[],
    })
  }
  async save(state: BeardStudioState,expectedRevision=state.revision) {
    const result = await this.client.rpc('save_beard_studio_workspace', { payload: beardStudioPayload(this.workspaceId, state,expectedRevision) })
    if (result.error) {
...
export function BeardStudioProvider({ children }: { children: ReactNode }) {
  ...
  return <BeardStudioContext.Provider value={{state,pending,error,retry:()=>window.location.reload(),update}}>{error&&<div className="form-error beard-runtime-error" role="alert"><strong>Save failed</strong><span>{error}</span></div>}{children}</BeardStudioContext.Provider>
}
