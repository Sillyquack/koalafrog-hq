...
export function IngredientsPage() {
  const data = useFormulaData(); const [search, setSearch] = useState(''); const [category, setCategory] = useState('All'); const [status, setStatus] = useState('All'); const [stock, setStock] = useState('All'); const [creating, setCreating] = useState(false)
  ...
  return <>
    <PageHeader eyebrow={`Material library / ${String(data.ingredients.length).padStart(2,'0')} records`} title="Ingredients" description="Material identity, sourcing context and lot-derived stock—kept deliberately separate." action={<button className="button primary" onClick={() => setCreating(true)}><Plus size={16}/>Add ingredient</button>} />
    ...
    {creating && <IngredientForm onClose={() => setCreating(false)} />}
  </>
}
