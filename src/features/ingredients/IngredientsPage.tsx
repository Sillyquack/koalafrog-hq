...
export function IngredientsPage() {
  ...
  return <>
    <PageHeader eyebrow="Material library / 08 records" title="Ingredients" description="Working knowledge of every oil, butter, wax and aromatic material in the workshop." action={<button className="button primary"><Plus size={17} />Add ingredient</button>} />
    <div className="metric-strip"><div><span>Materials</span><strong>{ingredients.length}</strong><small>Across 6 categories</small></div><div><span>Need attention</span><strong className="warning-text">{attention.length}</strong><small>At or below reorder level</small></div><div><span>Scent materials</span><strong>3</strong><small>In the current library</small></div></div>
