...
export function FormulaDetailPage() {
  ...
  return <>
    ...
    <PageHeader eyebrow={`${product?.name} / Formula workspace`} title={formula.name} description={formula.description} action={<Link className="button ghost" to={`/knowledge?tab=scent-memory&formulaVersionId=${selected.id}`}>Record Scent Memory</Link>} />
