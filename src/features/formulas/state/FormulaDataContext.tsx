  ...
  const value = useMemo<FormulaDataValue>(
    () => ({
      ...
      async createFormulaFromStudio(conceptId,input) {
        ...
        const issues=input.blankDraft?[]:formulationIssues(resolved.value.archetype.capabilities,input.phaseDefinitions??[],input.lines,input.manufacturingProcess)
        if(issues.length)throw new Error(issues.join(' '))
        const now=new Date().toISOString(),productId=uid(),formulaId=uid(),formulaVersionId=uid()
        const product:Product={id:productId,name:input.productName,category:input.productCategory?.trim()||resolved.value.template.productCategory,status:'Active',developmentStage:'Formulation',description:input.description,currentDevelopmentFormulaVersionId:formulaVersionId,scentProfile:'Product Studio concept',createdAt:now,updatedAt:now}
