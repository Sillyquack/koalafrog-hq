...
test('owner creates supplier drafts, records one external placement, leaves its sibling draft, and supersedes the plan',async({page})=>{
 ...
 await page.reload()
 await expect(page.locator('.placement-order-card').filter({hasText:'E2E-PLACED-1001'})).toContainText('Shipment 1 · delivery reported')
 expect((await client.from('purchase_order_confirmations').select('id').eq('purchase_order_id',placedOrder.id)).data).toHaveLength(1)
 expect((await client.from('purchase_order_shipments').select('status').eq('purchase_order_id',placedOrder.id)).data).toEqual([{status:'delivery_reported'}])
