...
async function seed(){
 ...
 for(const[index,component]of['bottle','closure'].entries()){
  ...
  await admin.from('packaging_inventory_movements').insert({...owned,id:`${p}-${component}-receipt`,packaging_inventory_lot_id:`${p}-${component}-lot`,type:'Receipt',quantity:30,unit:'pcs',reason:'Packaging receipt',notes:'',occurred_at:now,created_at:now})
 }
...
test('Production controlled-material workspace completes a multi-lot controlled lifecycle',async({page})=>{
 ...
 await page.reload()
 await expect(page.getByRole('heading',{name:'Authoritative balance'})).toBeVisible()
 await page.getByRole('link',{name:'Open canonical Traceability'}).click()
 await expect(page.getByRole('heading',{name:'Traceability',exact:true})).toBeVisible()
 await expect(page.getByRole('heading',{name:'Backward genealogy'})).toBeVisible()
 await expect(page.getByRole('heading',{name:'Raw-material lots'}).locator('..')).toContainText('Exact consumed quantity')
 await expect(page.getByRole('heading',{name:'Packaging lots'}).locator('..')).toContainText('bottle')
 await expect(page.getByRole('heading',{name:'Packaging lots'}).locator('..')).toContainText('closure')
 await expect(page.getByRole('heading',{name:'Current inventory impact'}).locator('..')).toContainText('Exact current on-hand')
 await expect(page.getByText('Confidence:',{exact:false}).first()).toContainText('Complete')
 await expect(page.getByRole('button',{name:/recall/i})).toHaveCount(0)
