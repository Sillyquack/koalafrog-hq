...
assert.equal(migrations.version, "1.1.0")
assert.deepEqual(migrations.migrations.map(item => item.orderedIndex), migrations.migrations.map((_, index) => index + 1))
assert.ok(migrations.migrations.every(item => /^[a-f0-9]{64}$/.test(item.sourceHash)))
assert.ok(migrations.migrations.every(item => !Object.hasOwn(item, "commit")))
assert.equal(migrations.provenance.introducingCommitResolution, "Derived at audit runtime with Git history; a new pre-commit migration is verified from the staged index; neither reference is persisted in this manifest.")
