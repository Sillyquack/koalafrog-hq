...
test("selects the newest pending instruction from durable history", () => {
  ...
  const comments = [
    { body: startBlock.replaceAll("proof-001", "material-001") },
    { body: startBlock.replaceAll("proof-001", "persistence-001") },
  ]
  const next = selectNextInstruction(issue, comments, {
    lastConsumedInstructionId: "persistence-001",
    runs: [
      { instructionId: "proof-001" },
      { instructionId: "persistence-001" },
    ],
  })
  assert.equal(next.instructionId, "material-001")
})

test("an audited pre-turn recovery can retry without a duplicate instruction", () => {
...
test("existing result comments suppress replay after state replacement", () => {
  const completed = formatCompletionPacket({
    instructionId: "proof-001",
...
test("owner gate stops affirmative production actions but ignores prohibitions", () => {
  ...
  assert.equal(
    ownerGateReason({
      ...control,
      prompt: "- no mutation of Supplier Offers, Purchase Plans, or inventory",
    }),
    null,
  )
  assert.equal(
    ownerGateReason({
      ...control,
      prompt:
        "- zero side effects on candidate, offer, Purchase Plan, PO, and inventory",
    }),
    null,
  )
  assert.match(
    ownerGateReason({
      ...control,
      prompt: "Purchase 20 units from the selected supplier.",
    }),
    /owner-gated action/,
  )
})
